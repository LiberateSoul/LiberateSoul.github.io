...
def send_message(to_user, access_token, region_name, weather, temp, wind_dir, note_ch, note_en, max_temp, min_temp,
    ...
    # 获取在一起的日子的日期格式
    know_year = int(config["know_date"].split("-")[0])
    know_month = int(config["know_date"].split("-")[1])
    know_day = int(config["know_date"].split("-")[2])
    know_date = date(know_year, know_month, know_day)
    # 获取在一起的日期差
    know_days = str(today.__sub__(know_date)).split(" ")[0]
    # 获取所有生日数据
    birthdays = {}
